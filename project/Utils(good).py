...
def Move(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10):
  xs = [x5, x6, x7, x8, x9, x10] # all points
  ...
  # calculate different between every point
    ...
    print(valid_xs)
    print(valid_xs_avg)
    if valid_xs_avg > 0 and valid_xs_avg <= 30:
      for now in valid_xs:
        dif_xs.append(-50)
    elif valid_xs_avg > 30 and valid_xs_avg <= 60:
      for now in valid_xs:
        dif_xs.append(-20)
    elif valid_xs_avg > 290 and valid_xs_avg <= 320:
       for now in valid_xs:
        dif_xs.append(50)
    elif valid_xs_avg > 260 and valid_xs_avg <= 290:
      for now in valid_xs:
        dif_xs.append(-20)
    else:
      prev = valid_xs[0]
  ...
  if len(dif_xs) == 0:
    ...
    if len(dif_xs) == 1 or len(dif_xs) == 2 or len(dif_xs) == 3 or len(dif_xs) == 4:
      for j in dif_xs:
        sum += j
      dif_avg = sum / len(dif_xs)
    elif len(dif_xs) == 5:
      sum = 0.8 * dif_xs[0] + 0.9 * dif_xs[1] + dif_xs[2] + 1.1 * dif_xs[3] + 1.2 * dif_xs[4]
      dif_avg = sum / len(dif_xs)
    elif len(dif_xs) == 6:
      sum = 0.7 * dif_xs[0] + 0.8 * dif_xs[1] + 0.9 * dif_xs[2] + 1.1 * dif_xs[3] + 1.2 * dif_xs[4] + 1.3 * dif_xs[5]
      dif_avg = sum / len(dif_xs)
    elif len(dif_xs) == 7:
      sum = 0.7 * dif_xs[0] + 0.8 * dif_xs[1] + 0.9 * dif_xs[2] + dif_xs[3] + 1.1 * dif_xs[4] + 1.2 * dif_xs[5] + 1.3 * dif_xs[6]
      dif_avg = sum / len(dif_xs)
    elif len(dif_xs) == 8:
      sum = 0.7 * dif_xs[0] + 0.8 * dif_xs[1] + 0.85 * dif_xs[2] + 0.95 * dif_xs[3] + 1.05 * dif_xs[4] + 1.15 * dif_xs[5] + 1.2 * dif_xs[6] + 1.3 * dif_xs[7]
      dif_avg = sum / len(dif_xs)
    elif len(dif_xs) == 9:
      sum = 0.7 * dif_xs[0] + 0.8 * dif_xs[1] + 0.85 * dif_xs[2] + 0.95 * dif_xs[3] + dif_xs[4] + 1.05 * dif_xs[5] +  1.15 * dif_xs[6] + 1.2 * dif_xs[7] + 1.3 * dif_xs[8]
      dif_avg = sum / len(dif_xs)
