...
RC카에서 가까운 7개의 이미지만 고려해서 RC카가 코너에서 필요 이상으로 일찍 턴하는 문제를 해결

각 이미지에서 트랙의 윤곽선, 무게중심점을 구함

무게중심점이 너무 가장자리에 있다면 무시해서 오차를 보정

트랙의 무게중심점이 하나도 없다면 back

무게중심점들의 x좌표간의 평균, 무게중심점들의 x좌표 간의 차이값의 평균을 더한 값을 가지고 방향을 정함

무게중심점들이 너무 한쪽으로 치우쳐졌을때 오차를 보정

방향은 직진, 좌회전, 강한 좌회전, 우회전, 강한 우회전, 후진

라즈베리파이가 아두이노에 방향값을 보낸 후 아두이노가 라즈베리파이에 이에 대한 ACK를 보내야 라즈베리파이가 다음 방향값을 보낼 수 있도록 함
